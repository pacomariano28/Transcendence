...
function TrackCard({
  ...
  if (!track) {
    return (
      <div className="min-w-0 rounded-3xl border border-dashed border-white/10 bg-black/20 p-5 transition-all duration-200 hover:-translate-y-0.5 hover:border-[#f7d046]/40 hover:bg-white/5 hover:shadow-[0_0_0_1px_rgba(247,208,70,0.08),0_12px_30px_rgba(0,0,0,0.18)]">
  ...
  return (
    <div className="group min-w-0 rounded-3xl border border-white/10 bg-black/20 p-5 transition-all duration-200 hover:-translate-y-0.5 hover:border-[#f7d046]/50 hover:bg-white/5 hover:shadow-[0_0_0_1px_rgba(247,208,70,0.12),0_18px_40px_rgba(0,0,0,0.22)]">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="break-words text-xs font-medium text-zinc-400 [overflow-wrap:anywhere]">
            {title}
          </div>
          <div className="mt-1 break-words text-[11px] uppercase tracking-[0.22em] text-zinc-500 [overflow-wrap:anywhere]">
            {subtitle}
          </div>
        </div>

        <div className="shrink-0 whitespace-nowrap rounded-full border border-[#f7d046]/20 bg-[#f7d046]/10 px-2.5 py-1 text-[11px] font-medium text-[#f7d046] transition-colors duration-200 group-hover:border-[#f7d046]/40 group-hover:bg-[#f7d046]/15">
...
function StatCard({
  ...
  return (
    <div
      className={`min-w-0 rounded-3xl border p-5 transition-all duration-200 hover:-translate-y-0.5 hover:border-[#f7d046]/50 hover:bg-white/5 hover:shadow-[0_0_0_1px_rgba(247,208,70,0.12),0_18px_40px_rgba(0,0,0,0.18)] ${toneClasses}`}
    >
      <div className="text-xs font-medium text-zinc-400">{label}</div>
      <div className="mt-2 break-words text-2xl font-semibold text-white transition-transform duration-200 hover:translate-x-0.5 [overflow-wrap:anywhere]">
        {value}
      </div>
    ...
    <div className="container-page min-w-0 py-10">
      <div className="card min-w-0 p-4 sm:p-6">
        <div className="flex min-w-0 items-start justify-between gap-4">
          ...
          <div className="shrink-0 rounded-2xl border border-white/10 bg-black/20 px-3 py-2 text-xs text-zinc-300 transition-all duration-200 hover:border-[#f7d046]/40 hover:bg-white/5">
            {t("profile.protected")}
          </div>
        </div>

        <div className="mt-6 grid min-w-0 grid-cols-[minmax(0,1fr)] gap-4 md:grid-cols-2">
          <StatCard label={t("profile.username")} value={user?.username ?? "-"}>
            <div className="text-xs text-zinc-500">
              {" "}
              {t("profile.user_email")}
            </div>
            <div className="break-words text-sm text-zinc-200 [overflow-wrap:anywhere]">
              {user?.email ?? "-"}
            </div>
          </StatCard>
        ...
        {hasSpotify ? (
              ...
              <div className="grid min-w-0 gap-4 md:grid-cols-2 xl:grid-cols-3">
                {spotifyProfile?.topArtists?.length ? (
                  spotifyProfile.topArtists.map((artist, index) => (
                    <div
                      key={artist.id}
                      className="group min-w-0 rounded-3xl border border-white/10 bg-black/20 p-4 transition-all duration-200 hover:-translate-y-0.5 hover:border-[#f7d046]/50 hover:bg-white/5 hover:shadow-[0_0_0_1px_rgba(247,208,70,0.12),0_18px_40px_rgba(0,0,0,0.2)]"
                    >
                      ...
                      <div className="mt-3 flex flex-wrap gap-2">
                        {artist.genres?.length ? (
                          artist.genres.map((genre) => (
                            <span
                              key={genre}
                              className="max-w-full break-words rounded-full border border-white/10 bg-white/5 px-2.5 py-1 text-xs text-zinc-300 transition-all duration-200 hover:-translate-y-0.5 hover:border-[#f7d046]/30 hover:bg-[#f7d046]/10 hover:text-zinc-100 [overflow-wrap:anywhere]"
                            >
                              {genre}
                            </span>
                          ))
          ...
          <div className="mt-4 min-w-0 rounded-3xl border border-dashed border-[#f7d046]/30 bg-black/20 p-6 text-center">
